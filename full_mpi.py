...
def looping_over_channels(i):
    #Taking search radius between consecutive channels to be 1 pixel = 2.8 arcsec considering the redshift range and beam size of the line cube:
    os.system("topcat -stilts tmatch2 matcher=sky params=3 in1='%s' ifmt1=ascii in2='%s' ifmt2=ascii join=1and2 find=best omode=out out='%s' ofmt=ascii values1='col1 col2' values2='col1 col2' fixcols=none"%(files3[i],files3[i+1],files4[i]))
    os.system("topcat -stilts tmatch2 matcher=sky params=3 in1='%s' ifmt1=ascii in2='%s' ifmt2=ascii join=1and2 find=best omode=out out='%s' ofmt=ascii values1='col1 col2' values2='col1 col2' fixcols=none"%(files3[i],files3[i+2],files5[i]))
   ...
   for x in range(len(lines1)):
       ...
       if len(lines)==0:
           ...
           for line in lines:
               ...
               DEC2=float(line2[1])
               Freq2=float(line2[2])
               #Taking search radius between all detection channels to be 2 pixels = 5.6 arcsec and total frequency width of the detection to be 10 channels with 30 kHz each channel width considering the small beam size of 7 arcsec and redshift coverage of frequency bandwidth:
               if (np.sqrt(((RA1-RA2)*np.cos(np.radians(DEC1),dtype=np.float128))**2 + (DEC1-DEC2)**2)*3.6E+3 <= 5.6) and (abs(Freq1-Freq2) < 10*abs(d_f)):
                   out.write(line)
           out.close()
...
#Slicing the full-size image cube into smaller image cubes of 12 pixel = 2.8*12 = 33.6 arcsec spatial length around the HI sources within its channel range of HI emission:
...
#for i in range(len(Xcenter)):
    ...
    sub_cube_i = cube.subcube(xlo=(Xcenter[i]-abs(h['CDELT1'])*6)*u.deg, xhi=(Xcenter[i]+abs(h['CDELT1'])*6)*u.deg, ylo=(Ycenter[i]-abs(h['CDELT2'])*5)*u.deg, yhi=(Ycenter[i]+abs(h['CDELT2'])*6)*u.deg, zlo=freq_min[i]*u.Hz, zhi=freq_max[i]*u.Hz)
...
#for fileID,fileName in enumerate(files_2):
    ...
    #If more than one source is found in moment-0 map, we select the nearest to the center.
        ...
        if len(dist)!=0:
           ...
           A_arcsec = None 
           B_arcsec = None
    if A_arcsec!=None and B_arcsec!=None:
